...
test('signed event delivery posts an allowlisted summary and dedupes retries', async () => {
  ...
  const body = JSON.stringify({
    ...
    payload: {
      ...
      confidentialField: 'must not reach Slack',
    },
  });
  const fields = {
  ...
  assert.doesNotMatch(slack.messages[0].text, /confidentialField|must not reach Slack/);
});
...
test('event delivery rejects cross-installation payloads despite a valid HMAC', async () => {
  ...
  const body = JSON.stringify({
    ...
    payloadSchemaVersion: 1,
    occurredAt: '2026-07-28T12:00:00.000Z',
    payload: { eventType: 'dataRecord.created' },
  });
